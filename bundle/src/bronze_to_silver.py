...
class BronzeToSilver:
    ...
    def _extract_config_vars(self):
        ...
        self.silver_airbnb_table_name = f"{self.catalog_name}.{self.silver_schema_name}.{self.silver_airbnb_table}"
        self.silver_rentals_table_name = f"{self.catalog_name}.{self.silver_schema_name}.{self.silver_rentals_table}"
    ...
    def _process_airbnb(self):
        ...
        # DatabricksLogger = self.DatabricksLogger
        ...
        df_bronze_airbnb = df_bronze_airbnb.withColumn('zipcode_formatted', self.Utilities.format_zipcode('zipcode'))
        ...
        df_silver_airbnb.write.mode("overwrite").option("mergeSchema", "true").saveAsTable(self.silver_airbnb_table_name)
        self.DatabricksLogger.info(
    ...
    def _process_rentals(self):
        ...
        df_bronze_rentals = df_bronze_rentals.withColumn("zipcode_formatted", self.Utilities.format_zipcode("postalCode"))
        ...
        df_rentals_clean.write.mode("overwrite").option("mergeSchema", "true").saveAsTable(self.silver_rentals_table_name)
        self.DatabricksLogger.info(
