...
class SilverToGold:
    """
    Class to handle the transformation from Silver to Gold layer in the data pipeline.
    """

    def __init__(self, bundle_root_path, dbutils, config_path, env="t"):
        """
        Initializes the SilverToGold transformer with necessary configurations and utilities.
        """
    ...
    def run(self):
        """
        Executes the transformation logic to convert Silver data to Gold tables.
        """
    ...
    def _process_airbnb(self):
        """
        Processes the Airbnb data from the Silver layer, performs necessary transformations and joins with postcode data, and writes the cleaned data to the Gold layer.
        """

        df_silver_airbnb = self.spark.table(self.airbnb_silver_table_name)
        df_gold_airbnb = (
            df_silver_airbnb.groupBy("postalcode4")
            .agg(
            ...
            .withColumn(
                "est_annual_airbnb_revenue",
                psf.round(
                    psf.col("avg_nightly_price")
                    * self.days_per_year
                    * self.occupancy_rate,
                    2,
                ),
        ...
        df_gold_airbnb = df_gold_airbnb.withColumn(
            "airbnb_revenue_rank", psf.rank().over(window_spec)
        )
        df_gold_airbnb = df_gold_airbnb.withColumn(
            "occupancy_rate", psf.lit(self.occupancy_rate)
        )
        df_gold_airbnb.write.mode("overwrite").saveAsTable(
            self.gold_top_zipcodes_airbnb_table_name
        )
        self.DatabricksLogger.info(
    ...
    def _process_rentals(self):
        """
        Processes the Rentals data from the Silver layer, performs necessary transformations and joins with postcode data, and writes the cleaned data to the Gold layer.
        """

        df_silver_rentals = self.spark.table(self.rentals_silver_table_name)
        df_gold_rentals = (
            df_silver_rentals.groupBy("postalcode4")
            .agg(
            ...
            .withColumn(
                "est_annual_rental_revenue",
                psf.round(psf.col("avg_monthly_rent") * self.months_per_year, 2),
            )
        )
        window_spec = Window.orderBy(df_gold_rentals.est_annual_rental_revenue.desc())
        df_gold_rentals = df_gold_rentals.withColumn(
            "rental_revenue_rank", psf.rank().over(window_spec)
        )
        df_gold_rentals.write.mode("overwrite").saveAsTable(
            self.gold_top_zipcodes_rentals_table_name
        )
        self.DatabricksLogger.info(
    ...
    def _process_investment_comparison(self):
        """
        Compares the estimated annual revenue from Airbnb and Rentals for each postal code, calculates the difference and ratio, and recommends an investment strategy based on which option is more profitable. The results are written to a Gold table.
        """

        rank_window = Window.orderBy(psf.col("revenue_airbnb_minus_rentals").desc())
        investment_comparison = (
            self.df_gold_airbnb.join(
                self.df_gold_rentals, on="postalcode4", how="inner"
            )
            .withColumn(
                "revenue_airbnb_minus_rentals",
                psf.round(
                    psf.col("est_annual_airbnb_revenue")
                    - psf.col("est_annual_rental_revenue"),
                    2,
                ),
            )
            .withColumn(
            ...
            .withColumn(
                "total_listings",
                psf.col("airbnb_listing_count") + psf.col("rental_listing_count"),
            )
            .withColumn(
                "recommended_strategy",
                psf.when(
                    psf.col("revenue_airbnb_minus_rentals") > 0, "Airbnb (Short-term)"
                ).otherwise("Kamernet (Long-term)"),
            )
            .withColumn("investment_rank", psf.rank().over(rank_window))
            .orderBy("investment_rank")
        )
        investment_comparison.write.mode("overwrite").saveAsTable(
            self.gold_investment_comparison_table_name
        )
        self.DatabricksLogger.info(
