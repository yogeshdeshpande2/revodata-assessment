...
# Read files from data/airbnb
df_airbnb = spark.read.option("header", True).csv("data/airbnb/airbnb.csv")
print("Top 5 rows from Airbnb data:")
df_airbnb.show(25)

airbnb_null_zipcode_count = df_airbnb.filter(psf.col('zipcode').isNull()).count()
print(f"Number of rows with null zipcode: {airbnb_null_zipcode_count}")
...
df_airbnb = df_airbnb.withColumn('zipcode_formatted', Utilities.format_zipcode('zipcode'))

print("Sample of formatted zipcodes:")
df_airbnb.select('zipcode', 'zipcode_formatted').show(25)

formatted_zipcode_null_count = df_airbnb.filter(psf.col('zipcode_formatted').isNull()).count()
print(f"Number of rows with null formatted zipcode: {formatted_zipcode_null_count}")

coords_df = df_airbnb.filter(psf.col('zipcode_formatted').isNull()).select("latitude", "longitude") \
    .dropna() \
    .dropDuplicates()
...
final_df = df_airbnb2.join(result2, on=["latitude", "longitude"], how="left")
